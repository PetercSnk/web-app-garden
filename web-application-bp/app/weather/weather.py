...
def format(day_id):
    day = Daily.query.filter(Daily.id == day_id).first()
    weather = Hourly.query.filter(Hourly.day_id == day.id).order_by(Hourly.time).all()
